import sys
from procedure import bb_should_fire, is_protecting_conveyor, is_protecting_conveyor_fast
...
def cardinal_pathfind_to(rc: Controller, sense: Sense, target: Position, going_home: bool) -> bool:
    ...
    cur_flat = cur_y * map_w + cur_x
    tgt_flat = tgt_y * map_w + tgt_x

    if cur_flat == tgt_flat:
        pf_state.past_pos = None
        return True

    # Start / restart A* if needed.
    if ((not pf_state.astar_active and not pf_state.result_path) or
            pf_state.goal != target or
            not pf_state.is_cardinal):
        print('got reset')
        pf_state.reset()
        ...
        pf_state.g_score[cur_flat] = 0
        heappush(pf_state.open_set, (0, cur_flat))
        if pf_state.computed_this_turn:
            return False
    ...
    if not pf_state.astar_active and pf_state.failed:
        pf_state.result_path = []
        print('cleared')
        return False  # BUG FIX: was bare `return` (None)

    if pf_state.result_path:
        # result_path now holds flat ints — one Position allocation here, outside any loop.
        next_flat = pf_state.result_path[0]
        next_x    = next_flat % map_w
        next_y    = next_flat // map_w
        next_pos  = POSITION_CACHE[next_flat]

        d = cur.direction_to(next_pos)

        # Possibly fix the conveyor on the tile the bot is currently standing on.
        conveyor_dir = d
        cur_bldg_id  = rc.get_tile_building_id(cur)
        entt         = rc.get_entity_type(cur_bldg_id)
        is_allied    = sense.is_allied_idxd(cur_flat)
        needs_fix    = (
            entt is None or not (
                entt == EntityType.CONVEYOR and
                rc.get_direction(cur_bldg_id) == conveyor_dir
            )
        ) and not (entt == EntityType.CORE and is_allied)

        if needs_fix and cur_flat != tgt_flat:
            if entt is not None:
                if is_allied:
        ...
        # Determine which direction the conveyor on the next tile should face.
        if going_home:
            if len(pf_state.result_path) > 1:
                conv_next_flat = pf_state.result_path[1]
                conv_next      = POSITION_CACHE[conv_next_flat]
        ...
        allied = sense.is_allied_idxd(next_flat)
        bb     = rc.get_tile_builder_bot_id(next_pos)
        if bb is not None and rc.get_id() != bb and not (sense.get_entity_idxd(next_flat) == EntityType.CORE and allied):
            print('blocked')
            return False  # BUG FIX: was bare `return` (None)

        # Build / destroy on next tile, then move.
        moved = False
        if allied and next_flat != tgt_flat:
            if rc.can_destroy(next_pos): rc.destroy(next_pos)
        else:
            if rc.can_fire(next_pos): rc.fire(next_pos)

        if next_flat != tgt_flat:
        ...
        if rc.can_move(d):
            rc.move(d)
            # BUG FIX: store next_pos (intended tile), not rc.get_position().
            # If this tile has a launcher, rc.get_position() returns the launched
            # destination — past_pos would then point at the wrong place and
            # silly_pathfind recovery would target the launch destination instead
            # of the tile the bot was supposed to occupy.
            pf_state.past_pos = next_pos
            moved = True

        if not moved:
            env    = sense.get_env_idxd(next_flat)
            entt   = sense.get_entity_idxd(next_flat)
            allied = sense.is_allied_idxd(next_flat)
...
def step_cardinal_astar_internal(rc: Controller, sense: Sense, max_expansions: int):
    ...
    _CONVEYOR          = EntityType.CONVEYOR
    _BRIDGE            = EntityType.BRIDGE
    _SPLITTER          = EntityType.SPLITTER
    _ARMOURED_CONVEYOR = EntityType.ARMOURED_CONVEYOR
    _BARRIER           = EntityType.BARRIER
    _ENTITY_COSTS_CONV = ENTITY_COSTS_CONVEYOR

    # Cardinals only — no diagonals.
    CARDINAL_TABLE = (
        ( 0, -1, -map_w),
        ( 0,  1,  map_w),
        ( 1,  0,  1    ),
    ...
    while open_set and expansions < max_expansions:
        ...
        if current == goal:
            if DEBUG_DRAW:
                rc.draw_indicator_dot(Position(current % map_w, current // map_w), 0, 255, 0)
            path = reconstruct_path(came_from, current)
            pf_state.result_path = path
            pf_state.result_path.pop(0)
            if DEBUG_DRAW and pf_state.result_path:
                for i in range(len(pf_state.result_path) - 1):
                    rc.draw_indicator_line(
                        Position(pf_state.result_path[i]   % map_w, pf_state.result_path[i]   // map_w),
                        Position(pf_state.result_path[i+1] % map_w, pf_state.result_path[i+1] // map_w),
        ...
        for dx, dy, d_delta in CARDINAL_TABLE:
            nxt_x = cx + dx
            nxt_y = cy + dy
            if nxt_x < 0 or nxt_x >= map_w or nxt_y < 0 or nxt_y >= map_h:
                continue
            ...
            if _is_in_vision(POSITION_CACHE[nxt]) and _get_tile_builder(POSITION_CACHE[nxt]) is not None and nxt != goal:
                continue
            if allied and (entt == _BRIDGE or entt == _SPLITTER or entt == _ARMOURED_CONVEYOR) and nxt != goal:
                continue
            if allied and entt == _CONVEYOR and nxt != goal:
                if not is_protecting_conveyor_fast(rc, sense, nxt, map_w, map_h):
                    continue

            cost = 1
            if not is_entt_pathable(entt, allied):
                if allied and entt == _BARRIER:
                    cost = _ENTITY_COSTS_CONV[entt][0]
                else:
                    continue
            ...
            # Inline manhattan — valid heuristic for cardinal-only movement.
            h = abs(nxt_x - goal_x) + abs(nxt_y - goal_y)

            came_from[nxt] = current
            g_score[nxt]   = tentative
            heappush(open_set, (tentative + h, nxt))

            if DEBUG_DRAW:
                rc.draw_indicator_dot(Position(nxt_x, nxt_y), 255, 0, 0)
