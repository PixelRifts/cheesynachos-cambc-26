...
class Gunner(Bot):
    ...
    def is_only_enemy_siphoning(self, target_pos: Position) -> bool:
        found_ally_siphon = 0
        found_enemy_siphon = 0
        not_enough_info = False
        for d in CARDINAL_DIRECTIONS:
            adj = target_pos.add(d)
            ...
            if self.rc.get_entity_type(bldg) in ENTITY_TRANSPORT:
                if self.rc.get_team(bldg) == self.rc.get_team():
                    found_ally_siphon += 1
                else:
                    found_enemy_siphon += 1
        return not_enough_info or (found_enemy_siphon > found_ally_siphon)
    ...
    def get_best_in_range(self, candidates):
        ...
        for p in candidates:
            ...
            if e is not None and self.rc.get_team(e) != self.rc.get_team():
                ...
                if entt == EntityType.HARVESTER and self.is_only_enemy_siphoning(p):
                    building_priority = 20
