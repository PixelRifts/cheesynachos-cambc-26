...
ENTITY_INFRASTRUCTURE_RESOURCE_SINK = { EntityType.CORE, EntityType.FOUNDRY, EntityType.GUNNER, EntityType.SENTINEL, EntityType.BREACH } | ENTITY_TRANSPORT
...
def is_protecting_conveyor_simple(rc: Controller, pos: Position) -> bool:
    ...
    going_bb = rc.get_tile_builder_bot_id(going_to)
    
    is_sink = rc.get_entity_type(going_bldg) in ENTITY_INFRASTRUCTURE_RESOURCE_SINK
    if is_sink and not going_bb is None: return True
    return False
...
def get_best_launcher_placable_with_diag(rc: Controller, a: Position, b: Position) -> Direction:
    best_score = -1
    best_dist = 10**9
    best_dir = Direction.CENTRE
    ...
    for d in DIRECTIONS:
        ...
        if not is_pos_quickly_turretable(rc, p): continue
        ...
        for nd in DIRECTIONS:
            adj = p.add(nd)
            if not is_in_map(adj, rc.get_map_width(), rc.get_map_height()):
                continue
