...
def compute_best_turret_dir_ored(rc: Controller, sense: sense.Sense, poi: Position, entt: EntityType, excludedir: Direction) -> Direction:
    max_dir = None
    max_score = -1
    ...
    for d in DIRECTIONS:
        ...
        attackables = rc.get_attackable_tiles_from(poi, d, entt)
        attackable_set = set(attackables)
        ...
        for t in attackables:
            ...
            is_ore = (env == Environment.ORE_TITANIUM or env == Environment.ORE_AXIONITE)
            if is_ore:
                score += 4

            # check adjacency ONLY within attackables
            for nd in CARDINAL_DIRECTIONS:
                adj = t.add(nd)
                if adj not in attackable_set:
                    continue

                adj_env = sense.get_env(adj)
                if adj_env == Environment.ORE_TITANIUM or adj_env == Environment.ORE_AXIONITE:
                    score += 8
    ...
    if max_dir is None:
        return poi.direction_to(enemy_core_pos)
