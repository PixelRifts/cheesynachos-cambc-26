...
class BuilderBot(Bot):
        ...
        for d in self.shuffled_cardinal_directions:
            ...
            if env == Environment.WALL or \
                ...
                if try_destroy(self.rc, self.sense, goto, adj, ti_min=get_ti_cost(self.rc, the_entt_to_use)+20):
                    dir = micro.compute_best_turret_dir_ored(self.rc, self.sense, adj, the_entt_to_use, d.opposite()) if the_entt_to_use != EntityType.CONVEYOR else d.opposite()
    ...
    def recompute_econ_connect_target(self):
        ...
        if self.econ_connect_protect_target is not None:
            ...
            if not self.is_launcher_protected(self.econ_connect_protect_target):
                dir = get_best_launcher_placable_with_diag(self.rc, self.econ_connect_protect_target, self.enemy_core_pos)
                if dir != Direction.CENTRE:
                    ...
                    if not try_destroy(self.rc, self.sense, self.econ_connect_protect_target, launcher_pos, ti_min=get_ti_cost(self.rc, EntityType.LAUNCHER)+10):
                        return
    ...
    def switch_to_possibly_patrol(self):
        ...
        patrol_condition = random.randint(0, 3) < 2
